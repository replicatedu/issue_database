...
impl ClassIssues {
    ...
    pub fn view_grades(self)  -> Result<Vec<ClassIssue>,()>{
        let body = self.requester.get_all_my_issues("grade_request".to_string()).expect("error closing");
    ...
    pub fn view_registrations(self)  -> Result<Vec<ClassIssue>,()>{
        let body = self.requester.get_all_my_issues("register".to_string()).expect("error closing");
        let deser: Vec<serde_json::Value> = serde_json::from_str(&body).expect("error parsinge");
        parse_to_vector(deser)
    }
    ...
    pub fn get_all_registrations(&self) -> Result<Vec<ClassIssue>,()>{
        let body = self.requester.get_all_issues("register".to_string()).expect("error closing");
        let deser: Vec<serde_json::Value> = serde_json::from_str(&body).expect("error parsinge");
        parse_to_vector(deser)
    }
    ...
    pub fn get_all_grade_requests(&self) -> Result<Vec<ClassIssue>,()>{
        let body = self.requester.get_all_issues("grade_request".to_string()).expect("error closing");
        let deser: Vec<serde_json::Value> = serde_json::from_str(&body).expect("error parsinge");
        parse_to_vector(deser)
    }
    ...
    pub fn post_grade(&self,request:ClassIssue,enc_feedback:&str) -> Result<(),()>{
        ...
        let body = self.requester.get_issue_comments(request.number).expect("error closing");
        let deser: Vec<serde_json::Value> = serde_json::from_str(&body).expect("error parsinge");
